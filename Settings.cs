...
namespace Spotifly
{
    public partial class Form1
    {
        ...
        private void ShowTutorial()
        {
            ...
            dialogResult = MessageBox.Show("You may press F2 or F3 so you rewind or advance 10 seconds respectively.", tutorialHeaderTxt, MessageBoxButtons.OKCancel);
            if (dialogResult == DialogResult.Cancel)
                return;

            dialogResult = MessageBox.Show("You may press F1 so you toggle \"Add to Queue\" button", tutorialHeaderTxt, MessageBoxButtons.OKCancel);
            if (dialogResult == DialogResult.Cancel)
                return;
        }
